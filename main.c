...
int main() {
    ...
    while (1) {
        ...
        if (ctx.event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) {
            Coordinate mouse = { .x = ctx.event.mouse.x, .y = ctx.event.mouse.y };
        ...
        if (ctx.event.type == ALLEGRO_EVENT_KEY_DOWN) {
            ...
            if (ctx.state == OPEN_MAP && ctx.event.keyboard.keycode == ALLEGRO_KEY_N && ctx.challenge_index <= 4) ctx.state = CHALLENGE;
        }
        ...
        if (ctx.redraw && al_is_event_queue_empty(ctx.queue)) {
            ...
            case OPEN_MAP:
                ...
                for (int i = 2; i >= 0; i--) {
                    ...
                    if (ctx.hunger_counter > i) al_draw_bitmap(ctx.imgs.hunger_filled, 5 + i * HUNGER_WIDTH, 250, 0);
                }
                break;
            case CHALLENGE:
